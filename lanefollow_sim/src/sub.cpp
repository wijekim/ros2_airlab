...
cv::Point left_pt(160,45);
cv::Point right_pt(480,45);
...
{   
    ...
    int error;
    error = TwoLineError(binary.cols,left_pt,right_pt);
    ...
    RCLCPP_INFO(node->get_logger(), "error: %d / Time:%.2f / Lvel:%.2f / Rvel:%.2f",error,totalTime,leftvel,rightvel);
}
