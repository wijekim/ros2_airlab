...
public:
  ...
  void send_waypoints()
  {
    if (!this->client_ptr_->wait_for_action_server(std::chrono::seconds(10))) {
      RCLCPP_ERROR(this->get_logger(), "Nav2 Action Server를 찾을 수 없습니다.");
      return;
    }
   ...
   /*std::vector<std::vector<double>> coords = {
    ...
    RCLCPP_INFO(this->get_logger(), "총 %zu개의 경유점으로 순환 주행을 시작합니다.", goal_msg.poses.size());
...
private:
  ...
  void result_callback(const GoalHandleFollowWaypoints::WrappedResult & result)
  {
    if (result.code == rclcpp_action::ResultCode::SUCCEEDED) {
        RCLCPP_INFO(this->get_logger(), "순환 주행 성공! 원점에 도착했습니다. 오차를 확인하세요.");
    } else {
        RCLCPP_ERROR(this->get_logger(), "주행 실패");
    }
  }
};
